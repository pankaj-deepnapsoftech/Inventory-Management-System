...
import "./Navigation.css";  // Assuming the styles are saved in a separate CSS file
...
const Navigation: React.FC = () => {
  ...
  return (
    <>
      ...
      <div className="absolute top-4 left-4 z-50 md:hidden">
        <div onClick={() => setCheckMenu(!checkMenu)}>
          {checkMenu ? (
            <IoCloseSharp
              size={30}
              className={`absolute left-[12rem] text-white ${showIcon ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-4"} transition-all`}
            />
          ) : (
            <RiMenu2Line size={30} className="text-white transition-all" />
          )}
        </div>
      </div>
      ...
      {/* Sidebar */}
      <div
        className={`bg-gradient-to-b from-[#262f3d] to-[#0f1018] backdrop-blur-xl text-white h-full overflow-auto scrollbar-hidden
         md:block ${checkMenu ? "block" : "hidden"}
         fixed z-40 w-64 md:w-auto top-0 left-0 px-3 py-3 transition-menu ${checkMenu ? "transform translate-x-0" : "transform translate-x-[-100%]"}`}
      >
